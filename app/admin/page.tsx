        ...
        <section className="booking-list">
          ...
          {visible.map((booking) => {
            ...
            return <article className="booking-card" key={booking.id}>
              ...
              <div className="booking-card-grid"><div><span>Address</span><p>{booking.address}, {booking.city}</p></div><div><span>Contact</span><p><a href={`tel:${booking.phone}`}>{booking.phone}</a>{booking.email && <><br /><a href={`mailto:${booking.email}`}>{booking.email}</a></>}</p></div><div><span>Notes</span><p>{booking.notes || "None"}</p></div>{booking.jobType === "maintenance" && <div className="full-field"><span>Genuine OEM parts lookup</span><p><strong>Equipment:</strong> {booking.equipmentMake} {booking.equipmentModel}<br /><strong>Engine:</strong> {booking.engineMake} {booking.engineModel}<br /><strong>Filter:</strong> {booking.filterType || "Not specified"}{booking.serialNumber ? <><br /><strong>Serial:</strong> {booking.serialNumber}</> : null}</p><div className="card-actions"><a className="button secondary small" href={partsLookupUrl(booking, "engine")} target="_blank" rel="noreferrer">OEM lookup by engine</a><a className="button secondary small" href={partsLookupUrl(booking, "equipment")} target="_blank" rel="noreferrer">OEM lookup by equipment</a></div><small className="muted">Use the official manufacturer catalog and verify the OEM part number before ordering.</small></div>}</div>
