    ...
    <main className="admin-shell">
      ...
      <section className="availability-panel">
        ...
        <div className="parts-lookup-form">
          ...
          <label>Engine horsepower (HP)<input placeholder="Example: 24 HP" value={partsEngineHorsepower} onChange={(event) => setPartsEngineHorsepower(event.target.value)} /></label>
        ...
        <section className="booking-list">
          ...
          {visible.map((booking) => {
            ...
            return <article className="booking-card" key={booking.id}>
              ...
              <div className="booking-card-grid"><div><span>Address</span><p>{booking.address}, {booking.city}</p></div><div><span>Contact</span><p><a href={`tel:${booking.phone}`}>{booking.phone}</a>{booking.email && <><br /><a href={`mailto:${booking.email}`}>{booking.email}</a></>}</p></div><div><span>Notes</span><p>{booking.notes || "None"}</p></div>{booking.jobType === "maintenance" && <div className="full-field"><span>Genuine OEM parts lookup</span><p><strong>Equipment:</strong> {booking.equipmentMake} {booking.equipmentModel}<br /><strong>Engine:</strong> {booking.engineMake} {booking.engineModel}{booking.engineHorsepower ? ` · ${booking.engineHorsepower}` : ""}<br /><strong>Filter:</strong> {booking.filterType || "Not specified"}{booking.serialNumber ? <><br /><strong>Serial:</strong> {booking.serialNumber}</> : null}</p><div className="card-actions"><a className="button secondary small" href={partsLookupUrl({ ...booking, equipmentType: booking.serviceName }, "engine")} target="_blank" rel="noreferrer">OEM lookup by engine</a><a className="button secondary small" href={partsLookupUrl({ ...booking, equipmentType: booking.serviceName }, "equipment")} target="_blank" rel="noreferrer">OEM lookup by equipment</a><a className="button secondary small" href={amazonOemPartsUrl({ ...booking, equipmentType: booking.serviceName })} target="_blank" rel="noreferrer">Search OEM parts on Amazon</a><a className="button secondary small" href="https://www.amazon.com/gp/cart/view.html" target="_blank" rel="noreferrer">Open Amazon cart</a></div><small className="muted">Use the official manufacturer catalog to verify the OEM part number before selecting an Amazon listing and adding it to your cart.</small></div>}</div>
