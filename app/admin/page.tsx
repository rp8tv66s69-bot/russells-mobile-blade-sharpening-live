...
function servicePrice(jobType: string, serviceId: string, bladeCount: number) {
  ...
  if (jobType === "chainsaw-sharpening") {
    return ({ "chainsaw-up-to-16": 15, "chainsaw-18-20": 20, "chainsaw-22-24": 25, "chainsaw-over-24": 30 } as Record<string, number>)[serviceId] || 0;
  }
  ...
  async function updateBladeService(
    ...
    const serviceDetail = normalizedJobType === "maintenance"
      ...
      : normalizedJobType === "chainsaw-sharpening"
        ? `${booking.barSize || "Bar size needed"}${booking.chainRemoval ? " · chain removal and reinstallation included" : " · chain supplied off the saw"}`
    ...
    await update(booking, {
      ...
      serviceDetail,
      price: servicePrice(normalizedJobType, booking.serviceId, normalizedBladeCount) + (normalizedJobType === "chainsaw-sharpening" && booking.chainRemoval ? 10 : 0),
    });
  }
