...
function partsLookupUrl(booking: Booking, lookupType: "engine" | "equipment") {
  const identifiers = lookupType === "engine"
    ? [
        ...
        booking.filterType,
        "OEM engine maintenance parts oil capacity oil filter air filter fuel filter spark plug parts diagram",
      ]
    : [
        ...
        booking.serialNumber,
        booking.serviceName,
        "OEM equipment maintenance parts manual parts diagram",
      ];
        ...
        <section className="booking-list">
          ...
          {visible.map((booking) => {
            ...
            return <article className="booking-card" key={booking.id}>
              ...
              <div className="booking-card-grid"><div><span>Address</span><p>{booking.address}, {booking.city}</p></div><div><span>Contact</span><p><a href={`tel:${booking.phone}`}>{booking.phone}</a>{booking.email && <><br /><a href={`mailto:${booking.email}`}>{booking.email}</a></>}</p></div><div><span>Notes</span><p>{booking.notes || "None"}</p></div>{booking.jobType === "maintenance" && <div className="full-field"><span>Parts lookup</span><p><strong>Equipment:</strong> {booking.equipmentMake} {booking.equipmentModel}<br /><strong>Engine:</strong> {booking.engineMake} {booking.engineModel}<br /><strong>Filter:</strong> {booking.filterType || "Not specified"}{booking.serialNumber ? <><br /><strong>Serial:</strong> {booking.serialNumber}</> : null}</p><div className="card-actions"><a className="button secondary small" href={partsLookupUrl(booking, "engine")} target="_blank" rel="noreferrer">Lookup by engine</a><a className="button secondary small" href={partsLookupUrl(booking, "equipment")} target="_blank" rel="noreferrer">Lookup by equipment</a></div><small className="muted">Verify part numbers against the manufacturer&apos;s parts diagram before ordering.</small></div>}</div>
