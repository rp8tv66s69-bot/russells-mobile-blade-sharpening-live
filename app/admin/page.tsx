...
function servicePrice(jobType: string, serviceId: string, bladeCount: number) {
  if (jobType === "maintenance") {
    return ({ "push-mower": 35, "riding-mower": 45, "zero-turn": 45, tractor: 80 } as Record<string, number>)[serviceId] || 0;
  }
...
export default function AdminPage() {
  ...
  async function updateBladeService(
    ...
    const normalizedJobType = ["blade-changing", "loose-blade-sharpening", "maintenance", "chainsaw-sharpening"].includes(jobType) ? jobType : "sharpening";
    ...
    const serviceDetail = normalizedJobType === "maintenance"
      ...
      : normalizedJobType === "chainsaw-sharpening"
        ...
        : `${normalizedJobType === "blade-changing" ? "Blade changing only" : normalizedJobType === "loose-blade-sharpening" ? "Loose/spare blade sharpening" : "Blade sharpening"} · ${normalizedBladeCount} ${normalizedBladeCount === 1 ? "blade" : "blades"}`;

    await update(booking, {
      jobType: normalizedJobType,
      bladeCount: normalizedBladeCount,
      serviceDetail,
      price: Math.max(30, servicePrice(normalizedJobType, booking.serviceId, normalizedBladeCount) + (normalizedJobType === "chainsaw-sharpening" && booking.chainRemoval ? 5 : 0)),
    });
  }
